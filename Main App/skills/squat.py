# AGENT SKILL: squat form analysis
...
class SquatDetector(BaseExercise):
    ...
    def process(self, landmarks):
        ...
        if key_landmark_visible:
            if knee_angle < 140 and self.stage != "down":
                # Enter "down" phase early so we can track shallow squats
                self.stage = "down"
                self.rep_issue = None  # Reset for new rep
        ...
        if self.stage == "down":
            ...
            # Track worst issue: if they haven't hit good depth yet, it's TOO HIGH
            if depth_status == "GOOD DEPTH":
                self.rep_issue = None
            elif depth_status == "TOO HIGH" and self.rep_issue is None:
                self.rep_issue = "TOO HIGH"
